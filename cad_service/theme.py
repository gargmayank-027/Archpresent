"""
cad_service/theme.py

Theme + palette system per the architecture doc §6. V1 MVP ships one fully
specced theme ("modern") end to end through the renderer (M16), matching
the MVP-checkpoint scope in the module plan (M01-M19). The other five
themes (Luxury, Scandinavian, Minimal, Traditional, Industrial) are listed
as metadata so the Next.js theme picker can show them as "coming soon"
without any renderer code change required to add them later — adding a
theme is meant to be a config addition, not a code change, and this file
is written so that remains true (see `list_themes()` / `resolve_theme()`).
"""
...
from cad_service.ir_models import RoomType, FurnitureCategory
...
_THEMES: dict[str, Theme] = {
    ...
    # Metadata-only placeholders — listed for the picker UI, not yet
    # renderable. Adding the real definition later requires no renderer
    # changes, per the module docstring above.
